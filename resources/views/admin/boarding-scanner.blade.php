...
@section('content')
    <div class="flex flex-col gap-6 max-w-3xl mx-auto w-full">
        ...
        <!-- Scanner Card -->
        <div
            class="bg-slate-900/50 backdrop-blur-md border border-slate-800 rounded-2xl p-8 relative overflow-hidden shadow-2xl">
            <div
            ...
            <div class="text-center mb-8">
                <div
                    class="w-20 h-20 bg-indigo-500/10 rounded-full flex items-center justify-center mx-auto mb-4 border border-indigo-500/20">
                    <i class="fa-solid fa-qrcode text-3xl text-indigo-400"></i>
                </div>
                <h2 class="text-2xl font-bold text-white mb-2">QR Token Scanner</h2>
                <p class="text-slate-400 text-sm">Scan the passenger's booking QR code or enter token manually to process
                    their boarding.</p>
            </div>
            ...
            <form id="scanner-form" class="space-y-6 relative z-10">
                <div>
                    ...
                    <div class="relative">
                        <input type="text" id="token-input" required
                            class="w-full bg-slate-800/80 border border-slate-700 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all font-mono text-center tracking-widest text-lg"
                ...
                <button type="submit" id="scan-btn"
                    class="w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-bold py-3.5 px-4 rounded-xl shadow-[0_0_20px_rgba(79,70,229,0.3)] transition-all flex items-center justify-center gap-2 text-lg">
            ...
            document.addEventListener('DOMContentLoaded', () => {
                ...
                form.addEventListener('submit', async (e) => {
                    ...
                    try {
                        ...
                        resultBox.classList.remove('hidden', 'bg-emerald-500/10', 'border-emerald-500/20',
                        ...
                        if (response.ok) {
                            ...
                            resultTitle.innerText = data.action === 'checkout' ? 'Check Out Successful' :
                                'Check In Successful';
                ...
                function onScanSuccess(decodedText, decodedResult) {
                    ...
                    document.getElementById('scan-status').className =
                        'text-center text-xs text-emerald-400 mt-2 font-bold';
                ...
                setTimeout(() => {
                    ...
                    if (readerEl) {
                        ...
                        if (btn) {
                            btn.className =
                                'px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded text-sm mb-2';
                        }
                    }
                }, 1000);
