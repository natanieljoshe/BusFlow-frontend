...
<div class="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-6 items-start">
    ...
    <!-- Balance Card -->
    <div class="bg-[rgba(4,5,15,0.6)] backdrop-blur-md border border-violet-500/20 rounded-2xl p-6 relative overflow-hidden shadow-[0_0_30px_rgba(168,85,247,0.05)]">
    ...
    <!-- Transaction History -->
    <div class="bg-[rgba(4,5,15,0.6)] backdrop-blur-md border border-violet-500/20 rounded-2xl p-6 relative shadow-[0_0_30px_rgba(168,85,247,0.05)]">
        ...
        @if(!empty($history))
            <div class="space-y-4 max-h-[500px] overflow-y-auto custom-scrollbar pr-2">
                ...
                <div class="flex items-center justify-between p-4 rounded-xl border border-slate-800 bg-slate-800/30">
                    <div class="flex items-center gap-4">
                        <div class="w-10 h-10 rounded-full flex items-center justify-center {{ $item['type'] === 'top_up' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400' }}">
                            <i class="fa-solid {{ $item['type'] === 'top_up' ? 'fa-arrow-down' : 'fa-arrow-up' }}"></i>
                        </div>
                        <div>
                            <p class="text-sm font-bold text-white">{{ $item['description'] ?? 'Transaction' }}</p>
                            <p class="text-xs text-slate-400 mt-0.5">{{ \Carbon\Carbon::parse($item['created_at'])->format('d M Y, H:i') }}</p>
                        </div>
                    </div>
    ...
    <div class="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-sm p-6 shadow-2xl transform scale-95 transition-transform duration-300" id="topup-modal-content">
        ...
        <form id="topup-form" class="space-y-4">
            ...
            <div class="flex justify-end gap-3 mt-8">
                <button type="button" onclick="closeTopupModal()" class="px-4 py-2.5 text-sm font-medium text-slate-300 hover:text-white transition-colors">Cancel</button>
                <button type="submit" id="submit-topup-btn" class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold rounded-lg shadow-lg transition-colors flex items-center gap-2">
                    Confirm
                </button>
            </div>
        </form>
